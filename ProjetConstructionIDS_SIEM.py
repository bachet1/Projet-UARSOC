...
""" Ici , il faudrait d'abord installer Docker,puis créer une base des données Cassandra, ensuite créerer une table ou keyspaceet vous importer les données.
...
# 2.2 :Fonction d'insertion des données d'alertes dans la base cassandr 
 ...
 def InsertionCassandra():# Cette fonction ne fonctionne pas encore
    def pandas_factory(colnames, rows):
        return pd.DataFrame(rows, columns=colnames)
    try:
        cluster = Cluster(['192.168.99.102'],port=3000)
