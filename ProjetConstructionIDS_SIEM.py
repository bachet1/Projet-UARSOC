...
# PARTIE N°2: Définition des fonctions et des modules

""" Ici , il faudrait d'abord installer Docker,puis créer une base des données Cassandra, ensuite créer une table ou keyspace et importer les données.
...
# ici, la fonction importe les 300 dernières enregistrements des données dans cassandra et les convertie en fichier DataFrame
...
def preparation_reshaping_modelisation_data(data,lags,model,n,THRESHOLD_Inf,THRESHOLD_Sup):
    start=time()
    scaler = MinMaxScaler(feature_range = (0, 1)) # Normalisation des données
    data_set= data['cpu'].values.reshape(-1,1)
    data_set_scaled=scaler.fit_transform(data_set)
    x_train = []
    y_train = []
    for i in range(lags,len(data_set_scaled)): # préparation des input du modèle
    ...
    y_pred=scaler.inverse_transform(model.predict(train)) # prediction des données  par le modèle
    y_or=scaler.inverse_transform(y.reshape(-1, 1))
    erreurs=y_or-y_pred # calcul des erreurs de prédictiondiction
    print("Erreur de prevision :%.2f"%(mean_squared_error(y_or,y_pred)*100)+'%') # affichage des erreurs de prévision sur la console
    anomalies=pd.DataFrame(index=data[lags:].datetime) # création d'une table des anomalies
    anomalies['THRESHOLD_Sup']=THRESHOLD_Sup
    anomalies['cpu_predit']=y_pred # insertion des cpu predit dans la table anomalie
    anomalies['cpu']= y_or # insertion des cpu origine dans la table anomalie
    anomalies['loss']=erreurs # insertion des erreurs dans la table anomalie
    anomalies.loc[anomalies['loss']<=THRESHOLD_Inf,'anomalie']=True # identification des anomalies
    anomalies.loc[anomalies['loss']>=THRESHOLD_Sup,'anomalie']=True
    anomalie=anomalies[anomalies.anomalie==True] # table ne contenant que des anomalies identiées
    elapsed=time()-start # calcul la durée des calculs de l'algorithme
    print("Durée d'exécution:%.2f"%(elapsed)) # affichage de la durée d'execution de calcul sur la console
...
# 2.3 :Fonction de Préparation,Modélisationlisation, Détectiontection et Visualisation d'anomalie en temps decallé pour la confirmation et ou le controle
...
# fonction pour l'autoencoder - idem que la précedente.
...
THRESHOLD=0.1 # seuil pour l'autoEncoder
# Définition des arguments: nous avons choisi deux argumentS dont un par defaut c'est-à-dire modèle temps reel
# la maintenance de l'IDS est assurée tous les 7 jours ce qui signifie que le télécharge ou met à jours jour le modèle tous les 7 jours
    ...
    while(1):
     ...
     while(1):
        ...
        while(1):
            ...
            sleep(5) # mise à jours toutes les 5 secondes
        sleep(604800) #pour la maintenance tous les 7 jours soit 24*60*60*7
